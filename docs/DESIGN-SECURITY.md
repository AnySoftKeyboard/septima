...
- **The Flow:** 1. The frontend retrieves the current JWT (JSON Web Token) from Firebase Auth.
  2. It passes this token in the `Authorization: Bearer <token>` header of the POST request.
  3. The Kotlin backend uses the Firebase Admin SDK to verify the token signature and extract the `uid`.
  4. The backend explicitly checks that the extracted `uid` matches the `userId` in the requested payload before enqueueing any Cloud Tasks.
