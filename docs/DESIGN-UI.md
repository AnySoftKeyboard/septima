...
- **Triggering Actions:** The UI uses lightweight REST `POST` calls (e.g., `/session/start`, `/session/message`) to enqueue background jobs. It does _not_ wait for the LLM response on this request.
