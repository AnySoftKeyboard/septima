...
*Read the specific DESIGN docs for deep implementation details.*
...
2. **Event-Driven UI:** Frontend uses REST `POST` to trigger jobs, but relies *only* on Firebase `onSnapshot` listeners to update the UI.
